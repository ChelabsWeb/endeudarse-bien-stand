...
- **Modo Sala (v2, tipo Kahoot)**: "Crear partida" genera el código (4 letras sin confundibles) y deja esa máquina de **proyector-host** mostrando el código gigante; los equipos entran con "Entrar con código", eligen nombre y **personaje** (12 stickers SVG propios, lock en vivo por presence, auto-asignación determinista al que no elige) y marcan **¡LISTOS!** (ready-check: viaja por presence; los equipos NO pueden arrancar — arranca el host recién con todos listos, y un doble toque del proyector fuerza el arranque si un equipo quedó colgado; la revancha no repite el ready-check). El código sigue siendo la semilla determinista (`xmur3`+`mulberry32`, semilla = `CODIGO#ronda`) y el canal Realtime pasa a **`bcu12m2:CODIGO`** (broadcast + presence, sin tablas).
  - **Rondas sincronizadas**: el host abre cada parada (`ronda {n,dur,tipo}`), la cierra cuando respondieron todos o vence SU reloj (los timers de las máquinas son cosméticos; el proyector muestra una **barra de tiempo** sincronizada al rondaT0 por delay negativo de CSS — se rearma solo al cambiar de ronda), y manda el **marcador ordenado** (`marcador {n,tabla,final}`): overlay de ~6s con FLIP de posiciones, count-up y flechas — grande en el proyector, con nota personal en las máquinas. El feedback pedagógico es la sala de espera (sin botón Seguir; 2,5s de lectura mínima). `prog` sale al decidir y lleva `rn/resp/av`. Ronda vencida = sin puntos + racha cortada. La bocha (índice 8, ver `BOCHA_IDX`) cierra al toque de todos sin marcador.
  - **Host resiliente**: proyector caído → releva la máquina de menor id; proyector nuevo a mitad de partida → adopta la ronda por heartbeats (5s, también mientras se decide) con 7s de gracia antes de poder cerrar por completitud; el host vigente encadena la ronda siguiente aunque el cierre lo haya mandado un host en retirada. "Saltar ronda" (que en la última parada muta a "Ver puntajes finales") y "Cerrar el año" como palancas del facilitador.
- **Cierre en el proyector**: al terminar el año, el proyector arma un **podio literal** — bloques 2º-1º-3º que se tocan, cada equipo parado arriba con su **medalla de oro/plata/bronce** (símbolos `#med-*`), corona y sticker grande al ganador, reveal de atrás para adelante; al podio suben solo los equipos que jugaron el año.
- **Teclado físico**: la pantalla de código/nombre acepta el teclado de la máquina además del de pantalla (letras/espacio, Backspace = ⌫, Enter = LISTO, Esc = volver).
...
- **Puntaje**: patrimonio + fondo×0,5 − intereses×1,5 − marcas×$8.000 (consultas de crédito: −$3.000 y NO bajan la categoría BCU) + rachas + medallas + **bonus por velocidad** (solo en sala y solo en decisiones BUENAS: hasta $2.000 que bajan linealmente contra el reloj de la ronda — apurarse a una mala no paga; `S.bonusVel`, se suma antes del `prog` para que el marcador lo incluya). ⚠️ `calcResumen` y `scoreParcial` deben mantenerse espejados (ambos suman `bonusVel`) o el podio no cuadra con la constancia.
